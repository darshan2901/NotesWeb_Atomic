function newElement() {
		...
		if (inputvalue === ''){
			...
			hideEmptyTxt = setTimeout(spanEmptytxt, 4000); // hide message in 4secs
		} else {
		...
		let spanTwo = document.createElement("span");//This is the Span edit button for each note
	    textTwo = document.createTextNode("✓");
	    spanTwo.className = "edit";
	    spanTwo.appendChild(textTwo);
	    li.appendChild(spanTwo);

		let span = document.createElement("span");// Spawn close button for each note
		text = document.createTextNode("\u00D7");
		span.className = "close";
		span.appendChild(text);
		li.appendChild(span);

		// We are storing the saved notes in Local Storage so that when you come after a long it is still there
		//We need to Get the editable element
		...
		// Hiding the saved note when we clicked close button
		...
		// We can edit the notes when we click edit button
		let hideMessage;
		...
		if (noteEditable == 'inherit' || noteEditable == 'false') {
			...
			textTwo.nodeValue = ':';
			// Writing a confirmation to the user
			...
			hideMessage = setTimeout(hideIt, 4000); // hide message in 4secs
		  }
	    }
	  }
	}
	// Make "Changes saved" disappear after 4 seconds
	function hideIt() {
		document.querySelector("#update").innerHTML = "";
		document.querySelector("#update").style.backgroundColor = "transparent";
		document.querySelector("#update").style.border = "none";
	}
	// Make "empty" disappear after 4 seconds
